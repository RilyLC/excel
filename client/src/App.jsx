...
const GlobalFilterItem = ({ item, columns, onUpdate, onRemove }) => {
  const listId = useId();
  const inputValue = (item.column || "").toLowerCase();
    ...
    if (inputValue) {
      matches = columns.filter(
        (c) =>
          ...
          (c.original && c.original.toLowerCase().includes(inputValue))
      );
    }
    // Limit to 50 items to prevent rendering lag
    return matches.slice(0, 50);
  }, [columns, inputValue]);

  return (
    <div className="flex items-center gap-2 p-2 bg-white border border-gray-200 rounded text-sm shadow-sm relative">
      {item.column === "_ALL_" && (
        <div className="relative flex items-center group">
          {/* 触发图标 */}
          <AlertTriangle size={16} className="text-orange-500" />
          <div className="absolute bottom-full  mb-2 px-3 py-2 w-48 bg-gray-800 text-white text-xs rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-150 ease-out pointer-events-none z-10">
            全列模糊搜索可能导致卡顿，请谨慎使用
          </div>
        </div>
      )}
...
const GlobalFilterGroup = ({
  ...
  return (
    <div
      className={`p-1 pt-2 rounded-lg ${
        depth > 0 ? "bg-gray-50/50 border border-gray-200 ml-4" : ""
      }`}
    >
      <div className="flex items-center gap-2 mb-2">
        <div className="flex items-center bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded px-2 py-1 gap-2 transition-colors">
          ...
          <button
            onClick={addItem}
            className="text-xs text-gray-600 hover:text-blue-600 flex items-center gap-1 px-1 rounded hover:bg-white transition-colors"
          >
            <Plus size={12} /> 添加条件
          </button>
          <button
          ...
          {depth > 0 && (
            <>
              ...
              <button
                onClick={onRemove}
                className="text-xs text-red-500 hover:text-red-700 flex items-center gap-0.5 px-1 rounded hover:bg-white transition-colors"
              >
                <Trash2 size={12} /> 删除组
              </button>
            </>
        ...
        {(group.items || []).map((item, idx) => (
          <div key={idx} className="flex items-start gap-3">
            <div className="mt-2.5 min-w-[32px] text-right shrink-0 select-none">
              {idx > 0 ? (
                <span
                  className={`text-[10px] font-bold px-1 py-0.5 rounded uppercase border ${
                    group.logic === "AND"
...
// Moved renderMatchingFields to top level to be shared
...
const SearchResultCard = ({
  ...
  return (
    <div className="bg-white rounded-lg shadow border border-gray-100 overflow-hidden transition-all hover:shadow-md">
      <div
        className="bg-white p-3 px-4 flex items-center justify-between cursor-pointer select-none border-b border-gray-100 hover:bg-gray-50/50"
      ...
      >
        <div className="flex items-center gap-3 overflow-hidden">
          ...
          <div className="flex items-center gap-2 overflow-hidden">
            <span className="font-semibold text-blue-600 text-lg truncate">
              {group.table}
            </span>

            {group.totalCount > 0 && (
              <span className="text-xs bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full font-medium whitespace-nowrap">
                {`找到 ${group.totalCount} 条`}
              </span>
            )}

            <div className="flex items-center gap-1 overflow-hidden">
              {group.matchReason &&
                group.matchReason.map((reason, i) => (
                  <span
                    key={i}
                    className="text-[10px] bg-yellow-50 text-yellow-700 border border-yellow-100 px-1.5 py-0.5 rounded truncate max-w-[100px]"
                  >
                    {reason}
                  </span>
                ))}
        ...
        <button
          ...
          className="ml-2 text-xs text-blue-600 hover:text-blue-800 hover:underline shrink-0 font-medium"
        >
          打开表格
        </button>
      </div>
                    ...
                    {group.matches.map((row, rIdx) => {
                      ...
                      return (
                        <tr
                          ...
                          onClick={() => onRowClick(group, row.id)}
                        >
                          {fields.map((field, fIdx) => (
                            <td
                              key={fIdx}
                              className="py-2 px-3 text-gray-600 group-hover:text-gray-900"
                            >
...
function App({ onLogout }) {
  ...
  const canDelete =
    ...
    currentUser?.permissions?.can_delete === true;
  ...
  const validateChangePasswordField = (field, value, allValues) => {
    ...
    if (field === "newPassword") {
      ...
      if (!PASSWORD_REGEX.test(v))
        return "新密码格式不正确（8-64位，至少包含字母和数字）";
      return "";
    }
    ...
    setChangePasswordForm((prev) => {
      ...
      setChangePasswordErrors((errPrev) => {
        ...
        if (field === "newPassword" && changePasswordTouched.confirmPassword) {
          errNext.confirmPassword = validateChangePasswordField(
            "confirmPassword",
  ...
  const submitChangePassword = async () => {
    ...
    const errs = {
      oldPassword: validateChangePasswordField(
        "oldPassword",
        oldPassword,
        changePasswordForm
      ),
      newPassword: validateChangePasswordField(
        "newPassword",
        newPassword,
        changePasswordForm
      ),
      confirmPassword: validateChangePasswordField(
        "confirmPassword",
        confirmPassword,
        changePasswordForm
      ),
    ...
    if (errs.oldPassword || errs.newPassword || errs.confirmPassword) {
      showAlert(
        errs.oldPassword || errs.newPassword || errs.confirmPassword,
        "warning"
      );
      return;
    }
  ...
  const [advancedSearchAllProjects, setAdvancedSearchAllProjects] =
    useState(true);
  const [
    advancedSearchIncludeUncategorized,
    setAdvancedSearchIncludeUncategorized,
  ] = useState(false);
  ...
  const globalFilterColumns = React.useMemo(
    () => [
      { name: "_ALL_", original: "(所有列) 提示:全列模糊搜索可能导致卡顿" },
  ...
  const executeDeleteProject = async () => {
    ...
    try {
      await api.deleteProject(
        deleteProjectState.project.id,
        deleteProjectWithTables
      );
      if (activeProject?.id === deleteProjectState.project.id)
  ...
  const handleUpdateTableProject = async () => {
    ...
    try {
      ...
      const localUpdate = {
        name: manageTableName,
        project_id:
          newProjectForTable === "null" ? null : Number(newProjectForTable),
      };
      ...
      setTables((prev) =>
        prev.map((t) =>
          t.id === manageTable.id ? { ...t, ...localUpdate } : t
        )
      );
    ...
    try {
      ...
      if (res.data && typeof res.data === "object" && "data" in res.data) {
        ...
        setSearchPagination((prev) => ({
          ...
          total: res.data.total,
          totalPages:
            res.data.totalPages || Math.ceil(res.data.total / pageSize),
        }));
      } else {
  ...
  const handleSearchResultClick = async (result, matchedRowId) => {
    ...
    try {
      ...
      const locateRes = await api.locateRow(
  ...
  const handleTableUpdate = async (updatedTableMeta) => {
    try {
      ...
      setTables((prev) =>
        prev.map((t) => (t.id === updatedTableMeta.id ? updatedTableMeta : t))
      );
    } catch (err) {
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setChangePasswordField("oldPassword", e.target.value)
                  }
                  onBlur={() => {
                    setChangePasswordTouched((prev) => ({
                      ...prev,
                      oldPassword: true,
                    }));
                ...
                />
                {changePasswordTouched.oldPassword &&
              ...
              <div>
                ...
                <input
                  ...
                  onBlur={() => {
                    ...
                    setChangePasswordErrors((prev) => {
                      const next = { ...prev };
                      next.newPassword = validateChangePasswordField(
                        "newPassword",
                        changePasswordForm.newPassword,
                        changePasswordForm
                      );
                      if (changePasswordTouched.confirmPassword) {
                        next.confirmPassword = validateChangePasswordField(
                          "confirmPassword",
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        setAdvancedSearchIncludeUncategorized(e.target.checked)
                      }
                      className="rounded text-blue-600 focus:ring-blue-500"
                    />
                    未分类
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {projects.length === 0 ? (
                      ...
                      projects.map((p) => {
                        ...
                        const checked =
                          advancedSearchProjectIds.includes(idStr);
                        return (
                          <label
                            key={p.id}
                            className="flex items-center gap-1 text-sm bg-white px-2 py-1 rounded border border-gray-200 shadow-sm"
                          >
                            <input
                              type="checkbox"
                              checked={checked}
          ...
          {searchResults ? (
            <div className="h-full overflow-y-auto">
              <div className="flex items-center justify-between gap-3 mb-4">
                <h2 className="text-xl font-bold truncate">搜索结果</h2>
              </div>
                ...
                <div className="space-y-6">
                  {searchResults.map((group, idx) => (
                    <SearchResultCard
                      ...
                      onTableClick={handleSearchResultClick}
            ...
            activeTable.type === "document" ? (
              ...
              <DataGrid
                ...
                onPageSizeChange={(ps) =>
                  loadTableData(activeTable, 1, filters, sorts, groups, ps)
                }
      ...
      {manageTable && (
                ...
                <button
                  onClick={() => {
                    if (
                      window.confirm(
                        `确定要删除这个${
                          manageTable.type === "document" ? "文档" : "表格"
                        }吗？`
                      )
                    ) {
                      api
                        .deleteTable(manageTable.id)
                        .then(() => {
                          if (activeTable?.id === manageTable.id)
                            setActiveTable(null);
                          setManageTable(null);
        ...
        >
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                {manageTable.type === "document" ? "文档名称" : "表格名称"}
              </label>
              <input
                type="text"
            ...
            <div>
              ...
              >
                ...
                {projects
                  .filter(
                    (p) =>
                      !manageTable.user_id || p.user_id === manageTable.user_id
                  )
                  .map((p) => (
            ...
            <div className="bg-gray-50 p-3 rounded text-xs text-gray-500 border border-gray-200">
              <p className="mb-1">
                <span className="font-semibold">物理表名:</span>{" "}
                {manageTable.table_name}
              </p>
              <p>
                <span className="font-semibold">创建时间:</span>{" "}
                {new Date(manageTable.created_at).toLocaleString()}
              </p>
            </div>
          </div>
        </Modal>
